...
class tunnel(threading.Thread):
    ...
    def get_socket_client(self):
        ...
        socket_client_request = socket_client.recv(self.buffer_size).decode('charmap')
        self.host, self.port = re.findall(r'([^\s]+):([^\s]+)', socket_client_request)[0]
    ...
    def certificate(self):
        self.log('Certificate:\n\n{}'.format(ssl.DER_cert_to_PEM_cert(self.socket_tunnel.getpeercert(True))))
    ...
    def tunnel_type_1(self):
        try:
            ...
            self.log('Connecting to {} port {}'.format(self.host, self.port))
            self.socket_tunnel.connect((self.host, self.port))
            self.log('Server name indication: {}'.format(self.server_name_indication))
    ...
    def tunnel_type_2(self):
        try:
            ...
            self.log('Connecting to remote proxy {} port {}'.format(self.proxy_host, self.proxy_port))
